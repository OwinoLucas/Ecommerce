...
class MpesaGateWay:
    ...
    def getAccessToken(self):
        try:
            res = requests.request("GET",self.access_token_url, auth=HTTPBasicAuth(self.consumer_key, self.consumer_secret))
    ...
    def stk_push(self, phonenumber, amount, callback_url):
        ...
        try:
            res = requests.request("POST",self.checkout_url, headers=self.headers, json=request_data, timeout=30)
